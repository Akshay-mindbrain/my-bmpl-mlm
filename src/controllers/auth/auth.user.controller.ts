...
import { loginServices } from "@/useCase/auth/Auth.user.service";
import { genAcessServices } from "@/useCase/auth/Auth.user.service";
...
export const genAccessToken = async (
  ...
  try {
    ...
    res.status(200).json({
      msg: "Access token generated successfully" , accessToken
    });
  } catch (error: any) {
    res.status(500).json({
