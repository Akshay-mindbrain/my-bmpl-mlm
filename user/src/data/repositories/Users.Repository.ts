...
export const updateUpline = async (
  ...
  // 1. Get ids from lineagePath (bottom → top)
  const ids = lineagePath.split(",").map(Number).reverse();

  // 2. Fetch only required users
  const users = await db.user.findMany({
    where: { id: { in: ids } },
    select: { id: true, legPosition: true },
  });
  const map = new Map(users.map(u => [u.id, u]));

  // 3. Traverse and collect valid uplines

  const validIds: number[] = [];

  for (let id of ids) {

    if (id === userId) continue; // skip self

    const user = map.get(id);

    if (!user) break;

    // allow null (root user) OR matching legPosition
    if (user.legPosition !== legPosition && user.legPosition !== null) break;
...
export const sumBVByLineage = async (pattern: string) => {
  ...
  const result = await prisma.bVLedger.aggregate({
    where: {
      user_id: { in: userIds },
    },
    _sum: {
...
export const sumIncomeByLineage = async (pattern: string) => {
  ...
  const result = await prisma.systemIncome.aggregate({
    where: {
      user_id: { in: userIds },
    },
    _sum: {
